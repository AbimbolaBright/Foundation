...
root = Tk()
root.title("Python: EBONNY STANDARD VENTURE")
...
def Database():
    ...
    cursor = conn.cursor()
    cursor.execute("CREATE TABLE IF NOT EXISTS `member` (mem_id INTEGER PRIMARY KEY AUTOINCREMENT NOT NULL, username TEXT, password TEXT, email TEXT, phone no TEXT)")
...
def LoginForm():
    ...
    lbl_register = Button(LoginFrame, text="Create Account", bg="green",font=('arial', 10))
    lbl_register.grid(row=5, columnspan=2, pady=10,sticky='WE')
...
def RegisterForm():
    ...
    password.grid(row=2, column=1)
    firstname = Entry(RegisterFrame, font=('arial', 20), textvariable=EMAIL, width=15)
...
def Register():
    ...
    if USERNAME.get == "" or PASSWORD.get() == "" or EMAIL.get() == "" :
        ...
        if cursor.fetchone() is not None:
            ...
            cursor.execute("INSERT INTO `member` (username, password, email,) VALUES(?, ?, ?, ?)", (str(USERNAME.get()), str(PASSWORD.get()), str(EMAIL.get()),))
            conn.commit()
            USERNAME.set("")
            PASSWORD.set("")
            EMAIL.set("")
...
def Login():
    ...
    if USERNAME.get == "" or PASSWORD.get() == "":
        ...
        if cursor.fetchone() is not None:
            lbl_result1.config(text="You Successfully Login", fg="yellow")
        else:
            lbl_result1.config(text="Invalid Username or password", fg="blue")
LoginForm()
